...
import { useUpdateClassForTeacher } from '@/features/teacher/hooks';
...
const ModalUpdateClass = () => {
  ...
  const open = isModalOpen('ModalUpdateClass');
  const { classId, classData } = getModalData('ModalUpdateClass') as {
    classId: string;
    classData: CreateClassFormData & { id: string };
  };

  const { mutateAsync: updateClass, isPending } = useUpdateClassForTeacher();
  ...
  // Prefill form với data hiện tại
  useEffect(() => {
    if (classData) {
      form.reset({
        name: classData.name,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>Chỉnh sửa thông tin lớp học</DialogDescription>
        </DialogHeader>
