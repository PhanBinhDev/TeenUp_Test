...
import { useUpdateStudent } from '@/features/parent/hooks';
...
export function ModalUpdateStudent() {
  const { isModalOpen, closeModal, getModalData } = useModal();
  const isOpen = isModalOpen('ModalUpdateStudent');
  const student = getModalData('ModalUpdateStudent') as Student | undefined;
  ...
  useEffect(() => {
    if (isOpen && student) {
      form.reset({
        name: student.name,
        dob: new Date(student.dob || student.dateOfBirth),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => closeModal('ModalUpdateStudent')}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Cập nhật thông tin học sinh</DialogTitle>
          <DialogDescription>
            Chỉnh sửa thông tin học sinh của bạn
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button
                type="button"
                variant="outline"
                onClick={() => closeModal('ModalUpdateStudent')}
                disabled={isPending}
              >
                Hủy
              </Button>
